...
int main(void) {
	int N, M;
	scanf("%d %d", &N, &M);		// N: 바구니 수, M: 공을 넣는 횟수

	int basket[101] = { 0 };	//바구니 배열 초기화 (0~100 인덱스 사용, 모두 0)

	//M번 공을 넣는 작업 수행
	for (int m = 0; m < M; m++) {
		int i, j, k;
		scanf("%d %d %d", &i, &j, &k); //i번부터 j번 바구니까지 k번 공을 넣음

		for (int b = i; b <= j; b++) {
			basket[b] = k;		//해당 범위의 바구니에 k번 공을 넣음
		}
	}

	//최종 바구니 상태 출력
	for (int b = 1; b <= N; b++) {
		printf("%d ", basket[b]);	//바구니에 들어 있는 공 번호 출력
	}
